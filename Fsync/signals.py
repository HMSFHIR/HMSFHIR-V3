...
def get_resource_id(instance):
    """Get appropriate resource ID based on instance type"""
    # For Patient model, use patient_id
    if hasattr(instance, 'patient_id') and instance.patient_id:
        return str(instance.patient_id)
    
    # For other models, check for specific ID fields
    if hasattr(instance, 'practitioner_id') and instance.practitioner_id:
        return str(instance.practitioner_id)
    ...
    try:
        ...
        for rule in sync_rules:
            try:
                ...
                logger.info(f"Queued {rule.resource_type} {resource_id} for {operation} (Queue ID: {queue_item.id})")
                
            except Exception as e:
                logger.error(f"Error queuing {rule.resource_type} {get_resource_id(instance)}: {e}")
        ...
        for rule in sync_rules:
            try:
                ...
                SyncQueue.objects.filter(
                    ...
                    operation='delete',
                    fhir_data={'resourceType': rule.resource_type, 'id': resource_id},
                    priority=50
                )
                
                logger.info(f"Marked {rule.resource_type} {resource_id} for deletion")
                
            except Exception as e:
                logger.error(f"Error marking {rule.resource_type} {resource_id} for deletion: {e}")
...
def connect_patient_signals():
    ...
    try:
        # UPDATED: Use your actual Patient model import path
        from django.apps import apps
        Patient = apps.get_model('Patients', 'Patient')  # Adjust app name as needed
        
        @receiver(post_save, sender=Patient)
        def patient_saved(sender, instance, created, **kwargs):
            logger.info(f"Patient signal triggered: {instance.patient_id} ({'created' if created else 'updated'})")
            handle_instance_save(sender, instance, created)
        
        @receiver(post_delete, sender=Patient)
        def patient_deleted(sender, instance, **kwargs):
            logger.info(f"Patient delete signal triggered: {instance.patient_id}")
            handle_instance_delete(sender, instance)
            
        logger.info("Manually connected Patient signals")
        
    except Exception as e:
        logger.error(f"Could not connect Patient signals: {e}")